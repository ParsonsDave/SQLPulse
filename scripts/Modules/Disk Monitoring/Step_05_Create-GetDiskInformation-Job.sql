...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'SQLPulse - Get Disk Information', 
		@enabled=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Powershell - Get Disk Information]    Script Date: 11/29/2025 3:55:06 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Powershell - Get Disk Information', 
		@step_id=1, 
...
$tableName    = "Disk_Information"
$eventTime = (Get-Date -AsUTC).ToString("o")
...
-- Now replace the value 'localhost' in the job with the actual instance name

    DECLARE @jobName SYSNAME = N'SQLPulse - Get Disk Information';
    DECLARE @stepName SYSNAME = N'Powershell - Get Disk Information';
